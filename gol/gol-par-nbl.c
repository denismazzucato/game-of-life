...
void exchange_column(
	...
	MPI_Irecv(&old[1], bwidth,	MPI_INT,
					  prec_node(), LAST_ROW_TAG, MPI_COMM_WORLD,
						reqRecvFirstRow);
	MPI_Irecv(&old[(bheight+1) * real_w + 1], bwidth, MPI_INT,
						next_node(), FIRST_ROW_TAG, MPI_COMM_WORLD,
						reqRecvLastRow);

	MPI_Isend(&old[real_w + 1], bwidth, MPI_INT,
						 prec_node(), FIRST_ROW_TAG, MPI_COMM_WORLD,
						 reqSendFirstRow);
	MPI_Isend(&old[bheight * real_w + 1],	bwidth, MPI_INT,
